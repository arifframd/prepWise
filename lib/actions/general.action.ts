...
// Membuat feedback berdasarkan interview yang diambil
export async function createFeedback(params: CreateFeedbackParams) {
  const { interviewId, userId, transcript } = params;

  try {
    const formattedTranscript = transcript.map((sentence: { role: string; content: string }) => `-${sentence.role}: ${sentence.content}`).join("");

    // Create feedback object dengan gemini
    const {
    ...
    // memasukkan feedback ke database
...
// Mengambil feedback berdasarkan interviewId
export async function getFeedbackByInterviewId(params: GetFeedbackByInterviewIdParams): Promise<Feedback | null> {
  ...
  // kenapa feedback.docs[0]? karena interview bisa selalu ditake dan akan selalu menghasilkan feedback baru
